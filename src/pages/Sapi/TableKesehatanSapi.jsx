...
const TableKesehatanSapi = () => {
  ...
  useEffect(() => {
    const fetchData = async () =>{
      const response = await fetch('http://localhost:8000/API/v1/Moomeadows/Kesehatan_Sapi')
  ...
  const handleInsertForm = async (e) => {
    ...
    const response = await fetch(
      "http://localhost:8000/API/v1/Moomeadows/Kesehatan_Sapi",
      {
        method: "POST",
  ...
  const handleDeleteRow = async () => {
    const response = await fetch(
      `http://localhost:8000/API/v1/Moomeadows/Kesehatan_Sapi/${selectDelete}`,
      {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
      }
          ...
          <div className="bg-white p-4 rounded w-[400px] flex flex-col gap-5">
            ...
            <div>
              <form className="flex flex-col gap-5" onSubmit={handleInsertForm}>
                <input
                  type="text"
                  placeholder="Id Kambing"
                  className="border-black border-2 rounded-xl p-2"
                  value={idsapi}
                  onChange={(e) => setIdSapi(e.target.value)}
                />
                <input
                  type="date"
...
export default TableKesehatanSapi;
