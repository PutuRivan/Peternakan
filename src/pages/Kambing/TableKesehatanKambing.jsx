...
const TableKesehatanKambing = () => {
  ...
  const [idkambing, setIdKambing] = useState("");
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch(
        "http://localhost:8000/API/v1/Moomeadows/Kesehatan_Kambing"
      );
  ...
  const handleDeleteRow = async () => {
    const response = await fetch(
      `http://localhost:8000/API/v1/Moomeadows/Kesehatan_Kambing/${selectDelete}`,
      {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
      }
  ...
  return (
    <div className="w-full bg-white shadow-lg rounded-lg p-6">
      <h1 className="text-center text-2xl py-3">KESEHATAN KAMBING</h1>
            ...
            <div>
              <form className="flex flex-col gap-5" onSubmit={handleInsertForm}>
                <input
                  ...
                  value={idkambing}
                  onChange={(e) => setIdKambing(e.target.value)}
                />
                <input
                  type="text"
